...
test("OpenAI submission packet has three Skills and exactly 5/3 reviewer cases", () => {
  assert.deepEqual(validateOpenAiSubmission(ROOT), {
    valid: true,
    public_skills: ["query-cuebook", "create-cuebook-content", "author-cuebook-skill"],
    positive_cases: 5,
    negative_cases: 3,
  });
});
...
test("OpenAI submission build emits a hashed three-Skill archive", () => {
  ...
  try {
    ...
    assert.deepEqual(manifest.public_skills, ["query-cuebook", "create-cuebook-content", "author-cuebook-skill"]);
