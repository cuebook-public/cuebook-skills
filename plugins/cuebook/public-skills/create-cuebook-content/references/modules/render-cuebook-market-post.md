...
> Module resources are rooted at `references/modules/render-cuebook-market-post/` from the public Skill directory.
...
4. If `ResearchPackV1` is supplied, validate it with `references/modules/build-market-research-pack.md`, preserve its source and fact IDs, and copy `quality_report.decision` into `research_decision`. If the request needs decision-grade comparators, valuation, positioning, or liquidity that the cue does not contain, build a research pack before drafting.
5. Compile or validate `MarketViewSemanticsV1`, then plan the expression with `references/modules/plan-cuebook-creator-expression.md`. This neutral layer locks claim type, subject, comparator, direction, horizon, mechanism, evidence breadth, speaker ownership, action tense, and settlement eligibility before prose changes shape.
...
13. Run four focused passes: semantic lock, evidence, creator lift, and Frame fit. Then score with `references/modules/render-cuebook-market-post/evals/rubric.md` and repair every hard failure.
14. When returning structured output, run `references/modules/render-cuebook-market-post/scripts/validate_post_artifact.mjs` and resolve all errors. Warnings require a deliberate review.
15. When the user accepts a settleable horizon or threshold, route the finalized prose to `references/modules/compile-cuebook-settlement-claim.md` only when the expression plan says settlement is eligible. Keep the contract as internal Frame metadata. Mention a deadline or level in the body only when it improves the argument; never display a separate settlement panel in the creative result.
...
Read `references/modules/render-cuebook-market-post/references/rendering-method.md` for evidence classes, angle selection, profile use, and Frame composition. Use `references/modules/render-cuebook-market-post/references/skill-matrix.md` as a market-situation reference, not as a substitute for source-first routing.
...
When called for explicit alternatives by `references/modules/assemble-cuebook-publish-candidates.md`, use one batch generation pass to render exactly three passed, Frame-ready `PostV1` siblings from the same expression fingerprint and fact ledger. Use distinct openings: conviction first, evidence first, and catalyst/condition first. Do not enter this mode for the default one-preview path.
...
- `references/modules/render-cuebook-market-post/templates/brief-template.md`: normalized request and source boundary.
- `references/modules/render-cuebook-market-post/references/rendering-method.md`: evidence, angle, profile, and Frame method.
- `references/modules/render-cuebook-market-post/references/post-v1.schema.json`: structured output contract.
- `references/modules/render-cuebook-market-post/references/skill-matrix.md`: market-situation examples and context needs.
- `references/modules/render-cuebook-market-post/scripts/validate_post_artifact.mjs`: deterministic PostV1 invariant checks.
- `references/modules/render-cuebook-market-post/evals/trigger_cases.json`: positive, negative, neighboring, and adversarial routing cases.
- `references/modules/render-cuebook-market-post/evals/expected_artifacts.json`: PostV1 contract expectations.
- `references/modules/render-cuebook-market-post/evals/rubric.md`: content quality gate.
- `references/modules/render-cuebook-market-post/evals/failure_cases.md`: stable regression cases from Cuebook.
