...
> Module resources are rooted at `references/modules/render-cuebook-market-media/` from the public Skill directory.
...
1. Resolve the assigned structured-media output from ContentRecipeV1. Apply channel, format, count, target context, language, flavor, and selected media/profile refs. For coordinated outputs, call `references/modules/plan-market-content-program.md`, render only the assigned item, and preserve recipe/program/content-item/opportunity/input-artifact refs in `lineage`.
...
8. Read `references/modules/render-cuebook-market-media/references/channel-methods.md`. Check official platform and named-community rules live for any `publish_ready` request; record URLs and check time in `policy_gate`. Record position, commercial relationship, identity, and AI-assistance disclosure states separately from generic risk language.
9. Create an asset plan with explicit rights status; do not reuse source media merely because it is public. Resolve data through `references/modules/assemble-cuebook-viewpoint-data.md`, then call `references/modules/render-cuebook-viewpoint-visual.md` for the primary compact argument and preserve its ref in both lineage and `asset_plan.artifact_ref`. Route only a long price history to `references/modules/render-cuebook-thesis-chart.md`, and only an oversized branching argument to the legacy visual-argument and logic-card pair.
...
```bash
node references/modules/render-cuebook-market-media/scripts/validate_media_package.mjs media-package-v1.json
```
13. Route the final `MediaPackageV1` to `references/modules/prepare-market-content-release.md`; this skill performs no account or platform operation.
...
- `references/modules/render-cuebook-market-media/references/channel-methods.md`: per-format structure and policy routing.
- `references/modules/render-cuebook-market-media/references/media-package-v1.schema.json`: MediaPackageV1 contract.
- `references/modules/render-cuebook-market-media/scripts/validate_media_package.mjs`: deterministic evidence, policy, timing, rights, and state checks.
- `references/modules/render-cuebook-market-media/evals/trigger_cases.json`: positive, neighboring, and adversarial routing cases.
