...
5. Capture both output sizes with `references/modules/direct-cuebook-viewpoint-visual/scripts/capture_html_viewpoint.cjs`; `viewpoint-2488.png` must be exactly 2488 x 1056 and `viewpoint-622.png` exactly 622 x 264, matching the direction's `preview_ref` and `compact_preview_ref`.
...
7. Run `references/modules/render-cuebook-viewpoint-visual/scripts/finalize_wide_viewpoint.mjs` to emit `ViewpointVisualV1` with `render_profile: wide_2488`, the selected `VDIR_` ref, HTML hash, production font-manifest hash, full PNG hash, compact PNG hash, palette preset, argument pattern, and source bindings. Freeze that manifest in the release artifact.
