...
> Module resources are rooted at `references/modules/plan-cuebook-creator-expression/` from the public Skill directory.
...
Read `references/modules/plan-cuebook-creator-expression/references/expression-protocol.md` before creating or revising a plan. Read `references/modules/plan-cuebook-creator-expression/references/visual-intent-routing-v1.md` and its registry before planning a visual. Read `references/modules/plan-cuebook-creator-expression/references/archetype-routing.md` when routing a commentator-style market view or testing one of the 11 source archetypes.
...
2. Copy the canonical claim, claim type, subject, comparator, direction, horizon, mechanism, source trade and settlement intent, action, supporting facts, caveats, and creator-owned experience refs into `meaning_fingerprint`. Separate the creator's explicit viewpoint, supported reasoning or hypothesis, observed facts, genuinely implied links, and missing links. The creator's viewpoint is the expression anchor, not an assertion to disprove by default. A causal bridge, analogy, scenario, intuition, or expectation may remain a creator-owned hypothesis without direct proof; only its factual premises must resolve to evidence. Never fill a missing link or resolve contradictory evidence by silently changing the claim. Calculate the canonical hash and lock both meaning and authorship.
3. Select the authorship mode:
   - `creator_original`: preserve the current creator's seed; add no Cuebook idea substance.
   - `cuebook_assisted`: preserve a real creator seed; record each Cue- or evidence-derived `evidence`, `connection`, `countercase`, or `rule` with its source ref and every creator acceptance or rejection; state the idea delta internally; keep rejected and unused Cue viewpoints outside the meaning fingerprint; keep assistance attribution out of public body copy.
...
6. Set `VoiceSpec`: language, register, energy, conviction, technicality, emotionality, compression, rhythm, humor, first-person stance and experience, technical terms, rhetorical devices, and optional profile rule refs. Enable anti-AI-language controls and ban locale-equivalent stock phrases for `it is worth noting`, `the core logic is`, and `from a mechanism perspective`; allow at most one `not A but B` frame.
...
10. Assess settlement eligibility without creating a contract. Keep source intent `none` fully ineligible; route only `candidate` or incomplete `explicit` intent to `references/modules/compile-cuebook-settlement-claim.md`.
...
12. Return `CreatorExpressionPlanV1` from `references/modules/plan-cuebook-creator-expression/references/creator-expression-plan-v1.schema.json` and validate it:

```bash
node references/modules/plan-cuebook-creator-expression/scripts/validate_creator_expression_plan.mjs creator-expression-plan-v1.json
```

To calculate the fingerprint hash while assembling a plan:

```bash
node references/modules/plan-cuebook-creator-expression/scripts/validate_creator_expression_plan.mjs creator-expression-plan-v1.json --print-fingerprint-hash
```
...
- Preserve creator intent. Cuebook may use aligned, contrasting, or adjacent Cues to strengthen, connect, narrow, or condition a creator-owned view; it may not silently replace it, treat another creator's stance as consensus, or add an unadopted Cue to first-person meaning. A material factual contradiction requires an explicit creator-facing decision; lack of direct proof for a clearly typed creator hypothesis does not.
...
- Keep the canonical Skill path and renderer pair aligned with the selected visual route. OHLC evidence requires `references/modules/render-cuebook-thesis-chart.md` as the optional detail renderer.
...
- Pass the same plan and fingerprint hash to `references/modules/render-cuebook-market-post.md` for Frame-sized body text. The candidate assembler derives the title and binds the selected image; no social or selector-only public derivative is created.
- Pass the same plan and fingerprint hash to `references/modules/compile-cuebook-visual-argument.md` before logic cards, market figures, or thesis charts when a graph-level argument is needed.
- Pass structured-media jobs to `references/modules/render-cuebook-market-media.md` after its data and asset requirements are satisfied.
- Pass settlement candidates to `references/modules/compile-cuebook-settlement-claim.md`; never mark a claim settled here.
...
- `references/modules/plan-cuebook-creator-expression/references/creator-expression-plan-v1.schema.json`: authoritative output schema.
- `references/modules/plan-cuebook-creator-expression/references/expression-protocol.md`: fingerprint, authorship, narrative, voice, text, visual, settlement, and firewall semantics.
- `references/modules/plan-cuebook-creator-expression/references/visual-intent-routing-v1.md`: intent-first Query, Skill, evidence-shape, renderer, and shortest-path protocol.
- `references/modules/plan-cuebook-creator-expression/references/visual-intent-route-registry-v1.json`: canonical candidate families, Query capability bindings, and visual Skill routes.
- `references/modules/plan-cuebook-creator-expression/references/archetype-routing.md`: routing for the 11 reverse-engineered source archetypes.
- `references/modules/plan-cuebook-creator-expression/scripts/validate_creator_expression_plan.mjs`: deterministic structural and cross-field validator.
