...
> Module resources are rooted at `references/modules/render-cuebook-market-figure/` from the public Skill directory.
...
2. Select one viewpoint visual grammar with `references/modules/render-cuebook-market-figure/references/viewpoint-visual-grammars.md`, then select the supporting curve grammar with `references/modules/render-cuebook-market-figure/references/curve-grammars.md`. For an explicit direction plus bounded horizon, default to `argument_curve` when a sourced series can show reaction, divergence, a confirmation level, expectation change, or pressure relevant to the bet. Continue here for a sourced sequence, synchronized comparison, or numeric formula. Route a pure price timeline to `references/modules/render-cuebook-thesis-chart.md`, pure causal prose to `references/modules/render-cuebook-logic-card.md`, a single decisive number or news item to `references/modules/render-cuebook-market-signal.md`, and an unsupported distribution or bridge to a missing-contract result. A curve is preferred when it proves something, not required by decoration policy.
3. Gather one chartable sequence, synchronized comparison, or explicit formula. A Skill is never a data source. Prefer Cuebook `MarketSeriesBatchV1`, `ThesisChartDataV1`, deterministic indicator history, estimate history, fundamental history, positioning/flow history, or a source-linked vehicle metric pack. When only an authorized chart image exists, follow `references/modules/render-cuebook-market-figure/references/source-chart-redraw.md`, mark every reconstructed series `digitized_observed`, and keep the figure conditional and non-settleable. Route missing non-OHLC evidence through `references/modules/build-market-research-pack.md`.
4. Run `references/modules/classify-cuebook-trading-logic.md` and copy its family, mechanism, expression, horizon, and public tags into `trade_logic`. Make the headline the creator's action or bet, not a workflow update.
5. When the viewpoint contains a decision-relevant mechanism or several confirmations, project two to four adjacent nodes from `VisualArgumentV1` into `argument_path`. Use `causal_chain`, `confirmation_ladder`, or `evidence_ladder`; preserve node status and source refs in the spec. Set `render.semantic_mode: argument_curve`. Prefer the sequence `changed input or catalyst -> actor action -> capital or market transmission -> creator bet`, omitting only a genuinely absent role. Name who reallocates, hedges, reprices, chases, cuts, or waits and, when supported, from which exposure toward which exposure. The public renderer turns node kinds into localized human roles such as `catalyst`, `forced actor`, `capital destination`, and `my bet`; evidence states stay machine-only.
...
9. Run the adapted visual review in `references/modules/render-cuebook-market-figure/references/visual-review.md`. Inspect the actual raster for hierarchy, type scale, contrast, crowding, truncation, currency, semantic color, and unnecessary motion or decoration. The review may simplify presentation; it cannot alter data or claims.
10. Rasterize only after SVG validation when a Feed PNG is needed, then repeat the visual review at final size.

```bash
node references/modules/render-cuebook-market-figure/scripts/validate_market_figure.mjs market-figure-spec-v1.json
node references/modules/render-cuebook-market-figure/scripts/render_market_figure.mjs market-figure-spec-v1.json --output-dir ./figure-output
node references/modules/render-cuebook-market-figure/scripts/rasterize_market_figure.cjs ./figure-output/market-figure.svg ./figure-output/market-figure.png
node references/modules/render-cuebook-market-figure/scripts/validate_market_figure.mjs ./figure-output/market-figure-v1.json --asset-root ./figure-output
```
...
- Make the curve carry one explicit argumentative verb: `break down`, `diverge`, `outperform`, `expand on volume`, `pull back`, `revise higher`, or another source-faithful relationship. At least one claim clause or node must point to its event, level, divergence, endpoint, or change; a curve that could be deleted without weakening the argument is decoration.
...
- For several ETFs or trade vehicles, use `instrument_map`: x is one common-window risk metric, y is one same-basis exposure metric, and every point names the instrument. Put leverage/reset, cost, or liquidity in sourced key numbers. Never inherit a creator's `conservative/aggressive` ordering without computing the shared risk axis.
...
- `references/modules/render-cuebook-market-figure/references/market-figure-spec-v1.schema.json`: semantic input and curve data.
- `references/modules/render-cuebook-market-figure/references/market-figure-v1.schema.json`: rendered asset manifest.
- `references/modules/render-cuebook-market-figure/references/viewpoint-visual-grammars.md`: routes KOL claims to curve, large-number, comparison, distribution, bridge, timeline, or logic-card forms.
- `references/modules/render-cuebook-market-figure/references/curve-grammars.md`: routing rules and required inputs.
- `references/modules/render-cuebook-market-figure/references/visual-review.md`: post-render hierarchy, typography, contrast, optical-alignment, and motion audit adapted from three MIT-licensed design skill sets.
- `references/modules/render-cuebook-market-figure/references/source-chart-redraw.md`: exact-series versus chart-redraw modes, multi-series focus rules, and the Cuebook leverage-indicator data contract.
- `references/modules/render-cuebook-market-figure/scripts/validate_market_figure.mjs`: structural and semantic validation.
- `references/modules/render-cuebook-market-figure/scripts/render_market_figure.mjs`: dependency-free Cuebook SVG renderer.
- `references/modules/render-cuebook-market-figure/scripts/rasterize_market_figure.cjs`: deterministic PNG rasterizer that follows the selected SVG dimensions.
