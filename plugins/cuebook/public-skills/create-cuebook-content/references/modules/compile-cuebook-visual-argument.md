...
> Module resources are rooted at `references/modules/compile-cuebook-visual-argument/` from the public Skill directory.
...
10. Validate with `references/modules/compile-cuebook-visual-argument/scripts/validate_visual_argument.mjs`.

## Commentator Post Route

Read `references/modules/compile-cuebook-visual-argument/references/kol-to-visual-routing.md` when the starting material is a public X/Twitter post, thread, Telegram post, article, or commentator corpus item.
...
- Send external facts, current prices, flows, positioning, and causal claims through `references/modules/build-market-research-pack.md`.
...
Use `references/modules/render-cuebook-logic-card.md` when the primary job is the graph itself and no quantitative sequence is needed. Use `references/modules/render-cuebook-market-figure.md` with `argument_curve` when two to four graph nodes should stay visible above a sourced price, estimate, fundamental, positioning, or comparison curve. Use `references/modules/render-cuebook-thesis-chart.md` for a pure `price_timeline` whose main job is level and settlement tracking.
...
Return `VisualArgumentV1` from `references/modules/compile-cuebook-visual-argument/references/visual-argument-v1.schema.json` and validate it:

```bash
node references/modules/compile-cuebook-visual-argument/scripts/validate_visual_argument.mjs visual-argument-v1.json
```

## Resources

- `references/modules/compile-cuebook-visual-argument/references/visual-argument-v1.schema.json`: shared semantic contract for every Cuebook opinion visual.
- `references/modules/compile-cuebook-visual-argument/references/kol-to-visual-routing.md`: KOL/commentator post classification, grammar routing, and evidence gates.
- `references/modules/compile-cuebook-visual-argument/scripts/validate_visual_argument.mjs`: graph, provenance, grammar, settlement, and state validator.
