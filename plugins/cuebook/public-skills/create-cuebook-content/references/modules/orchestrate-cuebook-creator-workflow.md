...
3. Retain only the selected direction. For `cuebook_template`, render with production fonts plus compact and visibility-required OG derivatives and run DOM/release audits once. For `finished_bitmap`, keep HTML/fonts optional and run the raster/image-review path once over publication, compact, and visibility-required OG pixels.
