...
> Module resources are rooted at `references/modules/orchestrate-cuebook-creator-workflow/` from the public Skill directory.
# Orchestrate Cuebook Creator Workflow

Operate the advanced release-grade control plane after a creator-facing preview exists. Reuse existing skills as capability nodes, preserve artifact lineage, and stop when a blocking gate fails. Never make the creator wait for this control plane before seeing a valid title, body, and image. Do not enter it for the ordinary publication of one selected Fast Preview; that path binds the preview directly through `FrameDraftAssemblyV1`.
...
4. Route every authenticated read through `references/modules/query-cuebook.md` and register its immutable `CuebookQueryBundleV1`; Create nodes consume bundle refs and never call read tools directly. Authorized writes remain separate Create actions.
...
2. **Intake**: run `references/modules/normalize-cuebook-creator-feed.md` and register `CreatorFeedV1`.
3. **Recipe and triage**: run `references/modules/compose-cuebook-content-recipe.md` before selection for `ingredient_first`/`preset_auto`, or after selection for `opportunity_first`; require exactly one `frame/publish_candidate_set` output and a visual branch, then register `ContentRecipeV1` and `ContentOpportunitySetV1`.
4. **Evidence route**: for each selected opportunity, run `references/modules/validate-cuebook-projection.md`, then `references/modules/route-cuebook-narrative.md`. Stop on projection reject or route abstention.
5. **Seed Query binding**: compile the smallest support request needed to verify the user's material current premises and safely compile semantics. Invoke `references/modules/query-cuebook.md`, then register the bundle, hash, cutoff, state, and selected result refs. Do not prefetch a broad visual package before the expression intent exists. A partial usable bundle makes dependent semantics conditional; a blocked material premise stops it.
6. **Research and semantics**: consume the bound query results and their referenced `ResearchPackV1` when research is required. Compile the research pack or direct creator/source input with `references/modules/compile-cuebook-market-view-semantics.md`. `MarketViewSemanticsV1` is the authoritative meaning and ownership boundary.
7. **Optional trade protocol**: only when the semantics contain explicit trade intent, resolve `references/modules/classify-cuebook-trading-logic.md` and/or user-selected `references/modules/compose-cuebook-trading-thesis.md`. Source-only, observation, explanation, and sentiment semantics skip both.
8. **Expression bridge and visual route**: run `references/modules/plan-cuebook-creator-expression.md` after semantics and any resolved trade protocol. A selected fast preview retains one chosen visual job; an explicit alternative request retains fast-read, proof, and system. Pass the meaning fingerprint and visual route hash unchanged downstream.
9. **Intent-driven gap Query**: compare `visual_plan.execution_route.query_requests` with every compatible current Query result. Reuse valid results whose entity, basis, cutoff, freshness, rights, and contract match. If any routed requirement remains unresolved, invoke `references/modules/query-cuebook.md` once with the smallest mixed gap request and register the new immutable bundle. Do not rerun Query when all routed requirements are already satisfied, and never substitute another tool for the locked capability.
10. **Program**: run `references/modules/plan-market-content-program.md` when the resolved recipe requires batches, event lifecycles, multiple outputs, derivatives, or a series.
11. **Parallel text and data**: run `references/modules/render-cuebook-market-post.md` in Frame-sized mode and `references/modules/assemble-cuebook-viewpoint-data.md` as sibling branches from the same expression plan and compatible Query bundles. Neither branch may depend on the other. The body carries the creator viewpoint, strongest observation, causal sequence, horizon, and next observable; the image data branch carries the relationship, reasoning beats, and timing when material.
12. **Selected viewpoint render**: execute the locked visual path only for retained directions. Choose `cuebook_template` for a reproducible Cuebook HTML render and keep licensed-font plus DOM audits; choose `finished_bitmap` for external/already-finished pixels and require only image inspection, exact 2488 × 1056 dimensions, encoded-byte/canonical-pixel hashes, bindings, alt text, and raster audit lineage. HTML or production Noi absence must not block `finished_bitmap`, and its generic `embedded-pixels-v1` profile never claims a font was verified from pixels. Three structurally different release directions are required only for an explicit alternative request. Use `references/modules/render-cuebook-thesis-chart.md` only when the selected route genuinely needs OHLC/K-line, long history, forming bars, explicit levels, or a settlement clock.
...
14. **Candidate freeze**: run `references/modules/assemble-cuebook-publish-candidates.md`. Preserve one selected preview without siblings, or assemble three only when explicitly requested. Build each exact public projection as `title + body + image`; keep all lineage and calibration backstage.
...
16. **Frame activation**: after explicit user intent, execute only the capability-advertised Frame sequence: upload the one publication master, register the manifest, create or update the draft, run `prepare_frame_publish`, and call `publish_frame` with its exact `prepared_hash` and `publish_token`. Do not request or poll separate action consent for initial publication. If a required B2/B3 tool is absent, stop at the latest completed phase without legacy fallback.
17. **Receipt terminal and warm handoff**: require and validate the publish receipt's versioned Frame ref, then stop the publication network flow. Do not call `get_frame`, browse or curl the canonical page, inspect web HTML or metadata, or expose a URL. Use a natural success line such as “已经替你发布好了，去 Cuebook App 看看吧。”, recognize one creator-specific kernel preserved by the frozen Meaning Lock, and ask at most one optional question. Prefer inviting the creator to share the idea from the App with another AI for a fresh judgment; the App-generated share entry must bind the receipt's exact `frame_id + release_id`, while the Skill exposes neither raw IDs nor `canonical_url`. Depending on context, the one question may instead ask about another signal or intuition. Keep it to two or three short sentences. For an eligible directional single-asset idea, a simulated Paper Trade may be offered only when its tools are available. The offer never authorizes a Tool call: wait for explicit opt-in, preview required order terms, then require explicit placement intent before `place_paper_order`. `get_frame` remains a Query capability for a later explicit read, never an automatic publication check.
18. **Reconcile and learn**: run `references/modules/reconcile-market-content-history.md` on receipts, revisions, corrections, engagement snapshots, and authorized outcomes.

Read `references/modules/orchestrate-cuebook-creator-workflow/references/workflow-map.md` for scenario paths, role gates, and skill ownership.
...
Do not use this full-contract path for an ordinary one-preview publication. The public Skill's direct Fast Publish lane validates the existing preview and builds Frame assembly without reconstructing release artifacts. Use this path only when an explicit advanced request requires the richer contracts:
...
- Candidate selection freezes the exact copy and visual. For the standard single-asset direction rule, explicit publish intent also confirms settlement and the receipt records every locked contract field internally; no second prompt or visible settlement form is allowed. A creator-specified target or pair remains `needs_confirmation` until its missing terms are confirmed.
...
Return `CreatorWorkflowRunV1` from `references/modules/orchestrate-cuebook-creator-workflow/references/creator-workflow-run-v1.schema.json`, then run `node references/modules/orchestrate-cuebook-creator-workflow/scripts/validate_creator_workflow.mjs`:
...
- `references/modules/orchestrate-cuebook-creator-workflow/references/workflow-map.md`: canonical stages, scenario paths, roles, and capability ownership.
- `references/modules/orchestrate-cuebook-creator-workflow/references/creator-workflow-run-v1.schema.json`: authoritative control-plane contract.
- `references/modules/orchestrate-cuebook-creator-workflow/scripts/validate_creator_workflow.mjs`: DAG, state, artifact, approval, and mode checks.
- `references/modules/orchestrate-cuebook-creator-workflow/scripts/build_example_bundle.mjs`: generate and validate a no-database six-contract example bundle.
- `references/modules/orchestrate-cuebook-creator-workflow/evals/trigger_cases.json`: routing cases.
- `references/modules/orchestrate-cuebook-creator-workflow/evals/rubric.md`: orchestration quality gate.
- `references/modules/orchestrate-cuebook-creator-workflow/evals/failure_cases.md`: stable orchestration failures.
