...
Do not materialize an internal workflow DAG for ordinary publication. Corrections and withdrawals use only their dedicated server actions in [Frame Publish Workflow](references/frame-publish-workflow.md); reproducibility audits remain separate developer work. Never create a public multi-image gallery. In the ordinary lane, the selected PNG is already the sole finished publication master; HTML, font files, and release derivatives are neither required nor generated.
