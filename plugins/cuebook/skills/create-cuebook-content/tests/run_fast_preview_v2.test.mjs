...
import { validateV2Job } from "../scripts/run_fast_preview_v2.mjs";
...
async function fakeHeaderRasterize(_svg, output) {
...
export function baseV2Job() {
  return {
    schema_version: "frame-preview-fast-job-v2",
    preview: {
      preview_id: "FPREV_BTC_FAST_V2_001",
      state: "conditional",
      ...
      candidates: [{
        candidate_id: "FPREV_CAND_BTC_FAST_V2_001",
        angle: "conviction",
        frame: {
    ...
    expressions: [{
      ...
      source_label: "合成测试序列",
      argument: {
        claim: { text: "抗跌不是结论，是轮动的起点", state: "creator_view", binding_id: "BIND_V2_CLAIM", source_refs: [] },
        observation: { text: "同期 BTC 相对 SPY 更强", state: "derived", binding_id: "BIND_V2_OBSERVATION", source_refs: ["RES_BTC_CANDLES", "RES_SPY_CANDLES"] },
        mechanism: { text: "我把它理解为持续承接正在吸收抛压", state: "creator_view", binding_id: "BIND_V2_MECHANISM", source_refs: [] },
        implication: { text: "先看韧性延续，再看它能否转成上冲", state: "conditional", binding_id: "BIND_V2_IMPLICATION", source_refs: [] },
        countercase: { text: "若相对强弱重新走低，这条推演失去支点", state: "conditional", binding_id: "BIND_V2_COUNTER", source_refs: [] },
      },
      observation_test: {
        kind: "primary_outperformed_benchmark",
        statement: "同期 BTC 相对 SPY 更强",
        threshold: 0,
        supports_binding_ids: ["BIND_V2_OBSERVATION", "BIND_V2_RELATIVE_CURVE"],
      ...
      market: {
        ...
        main_binding_ids: ["BIND_V2_BTC_CURVE"],
        support_binding_ids: ["BIND_V2_RELATIVE_CURVE"],
        chart_style: "candles",
        rolling_window: 10,
        show_volume: false,
      ...
      annotations: [{
        ...
        value: null,
        state: "derived",
        binding_id: "BIND_V2_REGIME",
        source_refs: ["RES_BTC_CANDLES", "RES_SPY_CANDLES"],
      }],
      future_beats: [
        { role: "checkpoint", label: "相对韧性继续保持", criterion: "5D 相对 SPY 强弱 > 0", at: "2026-07-31T09:00:00Z", state: "conditional", binding_id: "BIND_V2_CHECKPOINT", source_refs: [] },
        { role: "confirmation", label: "韧性转成价格上冲", criterion: "BTC 创 20D 新高", at: "2026-08-16T09:00:00Z", state: "conditional", binding_id: "BIND_V2_CONFIRM", source_refs: [] },
      ],
    }],
  };
}
...
function makeCreatorOnly(job, observation) {
  ...
  expression.argument.observation = {
    text: observation,
    state: "creator_view",
    binding_id: "BIND_V2_OBSERVATION",
    source_refs: [],
  };
...
test("V2 compiles one sourced curve, a derived support panel, and an honest future lane", async () => {
  const output = mkdtempSync(path.join(os.tmpdir(), "cuebook-fast-v2-"));
  try {
    const { preview, report } = await runFastPreviewJob(baseV2Job(), output, { rasterize: fakeRasterize });
    assert.equal(preview.candidates[0].visual_kind, "editorial_visual");
    assert.equal(preview.candidates[0].template_id, "curve_story");
    assert.equal(report.schema_version, "frame-preview-fast-run-report-v2");
    ...
    assert.match(svg, /data-binding-ref="BIND_V2_CHECKPOINT"/u);
...
test("V2 routes a drawdown argument to observed drawdown geometry", async () => {
  const output = mkdtempSync(path.join(os.tmpdir(), "cuebook-fast-v2-"));
  try {
    const job = baseV2Job();
    const expression = job.expressions[0];
    ...
    expression.market.main_binding_ids = ["BIND_V2_BTC_DRAWDOWN", "BIND_V2_SPY_DRAWDOWN"];
    ...
    setObservationTest(job, {
      ...
      supports: ["BIND_V2_OBSERVATION", "BIND_V2_BTC_DRAWDOWN", "BIND_V2_SPY_DRAWDOWN"],
    });
...
test("V2 can express conditional futures without inventing a market curve", async () => {
  const output = mkdtempSync(path.join(os.tmpdir(), "cuebook-fast-v2-"));
  try {
    const job = baseV2Job();
    const expression = job.expressions[0];
    ...
    expression.future_beats.push({
      ...
      at: "2026-08-08T09:00:00Z", state: "conditional", binding_id: "BIND_V2_INVALIDATION", source_refs: [],
    });
    const { preview } = await runFastPreviewJob(job, output, { rasterize: fakeRasterize });
    const svg = readFileSync(path.join(output, preview.candidates[0].candidate_id, "frame-preview.svg"), "utf8");
...
test("V2 rejects an unsupported causal claim masquerading as observed evidence", () => {
  const job = baseV2Job();
  job.expressions[0].argument.mechanism.state = "observed";
  const result = validateV2Job(job);
  assert.equal(result.valid, false);
  assert.ok(result.errors.some((error) => error.code === "BEAT_SOURCE"));
});

test("V2 rejects an observed sentence when the frozen curves contradict it", () => {
  const job = baseV2Job();
  setObservationTest(job, {
    kind: "benchmark_declined",
    statement: "同期 SPY 明显下跌",
    threshold: 0,
    supports: ["BIND_V2_OBSERVATION", "BIND_V2_RELATIVE_CURVE"],
...
test("V2 refuses a synthetic fixture that masquerades as a ready preview", () => {
  const job = baseV2Job();
...
test("V2 image_render requires a fully decodable, materially painted PNG", async () => {
  const output = mkdtempSync(path.join(os.tmpdir(), "cuebook-fast-v2-"));
  try {
    await assert.rejects(
      () => runFastPreviewJob(baseV2Job(), output, { rasterize: fakeHeaderRasterize }),
...
test("V2 rejects a future beat outside the declared horizon", () => {
  const job = baseV2Job();
...
test("V2 refuses exact title repetition inside the image", () => {
  const job = baseV2Job();
...
test("V2 rejects a pair transform when the benchmark is missing", () => {
  const job = baseV2Job();
...
test("V2 renders a sourced event window and keeps its event marker before declaration", async () => {
  const output = mkdtempSync(path.join(os.tmpdir(), "cuebook-fast-v2-"));
  try {
    const job = baseV2Job();
    const expression = job.expressions[0];
    ...
    expression.annotations = [{
      ...
      state: "reported", binding_id: "BIND_V2_EVENT", source_refs: ["RES_SPY_CANDLES"],
    }];
    setObservationTest(job, {
      kind: "primary_positive_after_event",
      statement: "事件发生后 BTC 仍保持正收益",
      supports: ["BIND_V2_OBSERVATION", "BIND_V2_BTC_CURVE", "BIND_V2_EVENT"],
      sources: ["RES_BTC_CANDLES", "RES_SPY_CANDLES"],
    });
    const { preview } = await runFastPreviewJob(job, output, { rasterize: fakeRasterize });
    const svg = readFileSync(path.join(output, preview.candidates[0].candidate_id, "frame-preview.svg"), "utf8");
    assert.match(svg, /data-composition="timeline_rail"/u);
    assert.match(svg, /data-binding-ref="BIND_V2_EVENT"/u);
  } finally {
    rmSync(output, { recursive: true, force: true });
  }
});

test("V2 renders an explicit threshold as state-changing geometry", async () => {
  const output = mkdtempSync(path.join(os.tmpdir(), "cuebook-fast-v2-"));
  try {
    const job = baseV2Job();
    const expression = job.expressions[0];
    ...
    expression.annotations = [{
      ...
      state: "creator_view", binding_id: "BIND_V2_THRESHOLD", source_refs: [],
    }];
    setObservationTest(job, {
      kind: "latest_above_threshold",
      statement: "BTC 最新一根日线仍在确认线之上",
      threshold: 70000,
      supports: ["BIND_V2_OBSERVATION", "BIND_V2_BTC_CURVE", "BIND_V2_THRESHOLD"],
      sources: ["RES_BTC_CANDLES"],
    });
    const { preview } = await runFastPreviewJob(job, output, { rasterize: fakeRasterize });
    const svg = readFileSync(path.join(output, preview.candidates[0].candidate_id, "frame-preview.svg"), "utf8");
    assert.match(svg, /data-composition="threshold_field"/u);
    assert.match(svg, /data-binding-ref="BIND_V2_THRESHOLD"/u);
  } finally {
    rmSync(output, { recursive: true, force: true });
  }
});

test("V2 compiles rolling correlation from synchronized returns without causal wording", async () => {
  const output = mkdtempSync(path.join(os.tmpdir(), "cuebook-fast-v2-"));
  try {
    const job = baseV2Job();
    const expression = job.expressions[0];
    ...
    expression.market.support_transform = "none";
    expression.market.main_binding_ids = ["BIND_V2_CORRELATION"];
    expression.market.support_binding_ids = [];
    expression.market.chart_style = "line";
    setObservationTest(job, {
      ...
      threshold: 0,
      supports: ["BIND_V2_OBSERVATION", "BIND_V2_CORRELATION"],
    });
    const { preview } = await runFastPreviewJob(job, output, { rasterize: fakeRasterize });
    const svg = readFileSync(path.join(output, preview.candidates[0].candidate_id, "frame-preview.svg"), "utf8");
    assert.match(svg, /data-chart-transform="rolling_correlation"/u);
    assert.match(svg, /data-binding-ref="BIND_V2_CORRELATION"/u);
  } finally {
    rmSync(output, { recursive: true, force: true });
  }
});

test("V2 renders a creator-owned causal spine without manufacturing a curve", async () => {
  const output = mkdtempSync(path.join(os.tmpdir(), "cuebook-fast-v2-"));
  try {
    const job = baseV2Job();
    const expression = job.expressions[0];
...
test("V2 enforces real structural diversity for three requested candidates", () => {
  const job = baseV2Job();
  ...
  secondCandidate.candidate_id = "FPREV_CAND_BTC_FAST_V2_002";
  ...
  secondExpression.market.main_binding_ids = ["BIND_V2_002_BTC", "BIND_V2_002_SPY"];
  ...
  thirdCandidate.candidate_id = "FPREV_CAND_BTC_FAST_V2_003";
  ...
  thirdExpression.market.main_binding_ids = ["BIND_V2_003_CORRELATION"];
...
test("V2 rejects future chart annotations, empty-clock beats, and reused preview bindings", () => {
  const futureAnnotation = baseV2Job();
  futureAnnotation.expressions[0].annotations[0].occurred_at = "2026-07-31T09:00:00Z";
  assert.ok(validateV2Job(futureAnnotation).errors.some((error) => error.code === "FUTURE_ANNOTATION"));

  const emptyClock = baseV2Job();
  emptyClock.expressions[0].time.future_mode = "empty_clock";
  assert.ok(validateV2Job(emptyClock).errors.some((error) => error.code === "EMPTY_CLOCK_BEATS"));

  const bindingCount = baseV2Job();
  bindingCount.expressions[0].market.support_binding_ids = [];
  assert.ok(validateV2Job(bindingCount).errors.some((error) => error.code === "SUPPORT_BINDING_COUNT"));
});
