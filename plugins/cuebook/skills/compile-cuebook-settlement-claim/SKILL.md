...
7. For a new single-asset Frame, use the exact creator-owned deadline. `long`/`short` use the standard zero-threshold comparison. `range` requires the creator-confirmed symmetric terminal band and maps to `single_asset_range`; it passes only when the absolute deadline return is within that band. Never translate “stays inside throughout” into terminal range. The server captures the publication baseline and later selects the authoritative completed price observation at or before the deadline; regular hours, trading days, and next close are not creator choices.
...
10. Validate with `scripts/validate_settlement_claim.mjs`. For a machine-settleable release, pass the ready or frozen claim to `$compile-cuebook-settlement-formula`; formula blockers propagate back to the claim.
