...
- Send external facts, current prices, flows, positioning, and causal claims through `$build-market-research-pack`.
...
Use `$render-cuebook-logic-card` when the primary job is the graph itself and no quantitative sequence is needed. Use `$render-cuebook-market-figure` with `argument_curve` when two to four graph nodes should stay visible above a sourced price, estimate, fundamental, positioning, or comparison curve. Use `$render-cuebook-thesis-chart` for a pure `price_timeline` whose main job is level and settlement tracking.
