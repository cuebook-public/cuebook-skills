...
for (const [name, expected, mutate] of [
  ...
  ["creator text cannot change", "CREATOR_TEXT_CHANGED", (item) => { item.blocks[0].summary = "View rewritten by AI"; }],
...
test("market figure ref must match lineage", () => {
  ...
  item.blocks.splice(5, 0, {
    ...
    title: "News and relative-return master visual", summary: "Bind the curve, news, key numbers, and settlement in one visual.",
...
test("viewpoint visual ref must match lineage", () => {
  ...
  Object.assign(item.blocks[2], {
    kind: "viewpoint_visual", title: "Where the risk premium lands first", artifact_ref: "VVIS_wrongref", display_variant: "viewpoint_visual",
  });
  assert.ok(codes(validate(item)).has("BLOCK_LINEAGE_REF"));
});
...
test("observation card can omit deadline and settlement", () => {
  ...
  Object.assign(item.header, {
    direction: "observe", direction_label: "Watch deleveraging", benchmark_instrument_id: null,
