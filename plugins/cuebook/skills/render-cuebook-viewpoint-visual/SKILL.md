...
Turn an approved visual direction into one exact 1866 x 1200 publication master. Author the HTML at 1244 x 800, use a 1.5x capture, and inspect that same composition at a 622 x 400 display size. Visual form is flexible; market meaning and source fidelity are not.
...
1. Require a selected `VisualDirectionSetV1`. If it does not exist, run `$direct-cuebook-viewpoint-visual` first and inspect its three real previews.
...
5. Capture one output with `$direct-cuebook-viewpoint-visual/scripts/capture_html_viewpoint.cjs`; `viewpoint-1866.png` must be exactly 1866 x 1200 and match the direction's `preview_ref`. Do not create compact, web, thumbnail, or OG files.
6. Inspect the publication PNG at full and 622 x 400 display sizes and run the director Skill's rendered audit. Confirm at least one `Cuebook Noi` face loaded, all visible non-brand copy leads with the approved stack, and CJK fallback did not change hierarchy or wrapping. Re-run the Cuebook visual critique after any typography, spacing, or composition change. Return to the layout Skill when the approved structure itself needs revision.
7. Run `scripts/finalize_wide_viewpoint.mjs` to emit `ViewpointVisualV1` with `render_profile: wide_1866`, the selected `VDIR_` ref, HTML hash, production font-manifest hash, publication PNG hash, palette preset, argument pattern, and source bindings. Freeze that manifest in the release artifact.
...
- Keep 2–3 essential prose groups, mark each with `data-essential-copy-group`, and keep their effective type at 18px or larger in the 622 x 400 audit.
...
Route to `$render-cuebook-thesis-chart` when OHLC, volume, multiple indicators, entry/invalidation/target geometry, or settlement tracking is the main evidence. A full chart can then become the hero primitive inside a directed composition; do not redraw it as a decorative line.
...
- Reject clipped text, unresolved placeholders, external network assets, and masters that fail either full-size or 622 x 400 display-scale inspection.
...
- `$direct-cuebook-viewpoint-visual`: default art-direction and three-preview workflow.
...
- `references/viewpoint-visual-v1.schema.json`: unified launch and compatibility output manifest; `wide_1866` is the publication profile.
