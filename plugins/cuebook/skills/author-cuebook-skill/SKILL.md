...
- Cuebook's own package identity lives in `../../assets/runtime-compatibility-v1.json` (`plugin_version`, `catalog_version`, and a binding `updates` block). Answer version questions from that manifest, and leave updating to the host's own update path followed by a host restart; never self-update or reinstall from this Skill.
