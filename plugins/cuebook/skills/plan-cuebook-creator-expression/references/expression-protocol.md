...
- Keep invalidation and countercase in the semantic contract. Surface either in body copy only when explicitly requested by the user or selected format; translate it into a neutral watch or change in confidence, never `invalidation`, `falsification condition`, `I admit I was wrong`, `where I was wrong`, `what if I am wrong`, or locale equivalents.
- For the public Frame, use one title, one reasoned body, and one paired image. Choose an adaptive body shape—usually two to seven paragraphs and roughly 160–1,100 visible Chinese characters (about 70–450 English words), with a 1,200-character hard Frame ceiling. Treat these as capacity bounds rather than a house rhythm. The body carries the strongest observation, creator viewpoint, and material causal sequence; horizon or next observable appears only when it improves the argument. The image carries the relationship, selected reasoning beats, observed evidence, and a timing marker when material. Do not duplicate the complete argument across both surfaces or pad a simple idea merely to hit a target.
...
- `candidate` may route to `$compile-cuebook-settlement-claim`; list missing metric, operator, threshold, deadline, or authoritative source fields.
