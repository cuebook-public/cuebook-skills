...
- Use only the host-installed `cuebook` MCP connector. Do not run a CLI login from this Skill—especially after a transport failure—implement OAuth discovery or DCR, exchange tokens, create a custom client, store credentials in task files, open another task, or retry automatically.
