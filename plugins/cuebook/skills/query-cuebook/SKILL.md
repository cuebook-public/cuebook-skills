---
name: query-cuebook
description: Read, compare, visualize, or explain Cuebook market intelligence without writes. Use for assets, Cues, Frames, market state, evidence, fundamentals, valuation, OHLCV, indicators, factual curves, settlement, community Skill discovery or provenance, commentator research, and explicit decision-memory reads. When separately connected and explicitly requested, add one bounded TradingView observation, focused capture, or network research check. Return a sourced CuebookQueryBundleV1. Do not draft or publish Frames, mutate charts or memories, install Skills, or call write tools. Frame and memory writes belong to create-cuebook-content; explicit submission of a creator-authored package with a root SKILL.md belongs only to author-cuebook-skill.
license: Proprietary. Cuebook internal; see the repository README for terms.
compatibility: Uses a connected Cuebook MCP server plus one bounded authorized Web lane. Optional, separately installed tradingview_desktop and tradingview_research connectors add local-chart and network research without becoming dependencies. Degrades to partial results, never invented values. Node.js 18+ for validators.
---
...
For an explicit or accepted TradingView aid, read `references/tradingview-workbench.md` (chart images: `references/tradingview-focused-capture.md`); an absent connector never blocks Cuebook.

Explicit personal memory/replay asks: `references/decision-memory-coach.md` (consent step-up; read-only).
...
- Package identity lives in `../../assets/runtime-compatibility-v1.json`: `plugin_version` names the installed package, `catalog_version` its capability catalog, and the `updates` block is binding. When the creator asks what version is installed or whether to update, answer from that manifest instead of guessing. Updates run only through the host's own update path and load at the next host reload; the active session keeps its startup snapshot, so an update also means restarting the host and continuing in one new task. Never self-update or reinstall from this Skill, and treat a capability-tier expansion or major version change as waiting for the creator's explicit confirmation.

## Routing

1. Normalize once with `references/cuebook-intent-v1.schema.json`: route by effect, order mixed dependencies, and default ambiguity to one read-only Query answer.
...
5. Select the smallest query path that answers the request:
   ...
   - curves or triggers: `get_candles`; preserve its raw frozen envelope for a creation handoff, and invoke `$compute-cuebook-market-indicators` only when a requested indicator is actually needed;
   ...
   - settlement outcomes: `list_settlements` and `$reconcile-market-content-history` when a history ledger is requested;
...
Normalize cross-surface intent first, then the Query step with `references/cuebook-query-request-v1.schema.json`. Return the contract in `references/cuebook-query-bundle-v1.schema.json`. Validate it with:

```bash
node scripts/validate_cuebook_intent.mjs cuebook-intent-v1.json
node scripts/validate_query_request.mjs query-request-v1.json
node scripts/validate_query_bundle.mjs query-bundle-v1.json
```

When used, validate the workbench with `node scripts/validate_tradingview_observation.mjs tradingview-observation-v1.json`; validate a requested chart image with `node scripts/validate_tradingview_focused_capture.mjs tradingview-focused-capture-v1.json`.

Use `../../assets/query-menu-v1.json` for product-facing query types and `../../assets/cuebook-modules-v1.json` for the enforced module boundary.
