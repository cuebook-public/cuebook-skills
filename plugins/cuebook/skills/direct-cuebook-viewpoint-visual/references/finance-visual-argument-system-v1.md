...
Composition answers how the argument is read at 622 x 400. It must not override chart integrity.
...
- A value directly labeled on evidence geometry is already stated. Restating it at display scale (>= 40 authored pixels) in another role group is a repeated fact and fails the rendered audit (`VALUE_RESTATED`); smaller restatements are flagged for reverse deletion.
- The hero slot next to a proof carries the judgment or the derived transform result — the delta, percent change, spread, or gap that the geometry cannot label as directly — never a copy of an endpoint the chart already labels.
- `data-value-restate-ok="true"` marks the one legitimate exception: the restated value is itself the claim and the geometry proves its composition or structure rather than the same scalar.

### Tracks, remainders, and containers

- A background track, remainder bar, capacity outline, or 100% container asserts a denominator. Without a reconciled denominator binding, comparable bars sit on a shared zero baseline with no track.
- Never scale a track to the largest bar for visual tidiness; that invents a part-to-whole relationship.

### Ordered axes and unequal gaps

- When time or tenor distance is part of the argument (front-loaded maturities, refinancing walls, reaction windows), equal spacing over unequal gaps misstates the structure.
- Either scale positions by time, or keep the ordinal layout and make the gap visible: an axis break, a labeled empty band, or explicit interval labels.
- Record the choice in `expression_recipe.axis_integrity` (`time_scaled`, `ordinal_gap_marked`, or `uniform_true`); the validator requires it for grammars with an ordered dated axis.

### Delta and summary annotations

- A derived delta, spread, or group-summary label attaches to the geometry it measures: bracket both marks or sit in the gap between them. A label placed beside only one mark misattributes the quantity.
- Annotation text never crosses its own bracket, rule, or axis stroke; the rendered audit rejects any text crossing a rendered border line (`TEXT_BORDER_COLLISION`).
- A summary annotation inherits the color role of the group it summarizes, or stays neutral ink. It never introduces a new hue.
- The declared `finance_transform` result is the analytical payoff of the image; render it at least at context-label size, never as the smallest text on the canvas.
...
Compose the 622 x 400 reading first, then author the 1244 x 800 version with the same hierarchy.
...
- A value already labeled on the proof geometry is restated as a display-scale hero number in another group.
- A background track or remainder bar implies a denominator that no binding reconciles.
- Equal spacing hides a materially unequal tenor or time gap that the argument depends on.
- A delta or summary annotation sits beside one mark instead of bracketing the two marks it compares, or introduces a hue foreign to the group it summarizes.
