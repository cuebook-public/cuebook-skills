...
node scripts/validate_visual_direction_set.mjs visual-direction-set-v1.json \
  --require-expression-recipes --require-finance-route
node scripts/stamp_cuebook_wordmark.mjs direction-a.html --background dark
node scripts/lint_launch_viewpoint_html.mjs direction-a.html
...
- Full OHLC, volume, or indicator analysis routes to `$render-cuebook-thesis-chart` and can be embedded as the evidence hero.
...
- Run `scripts/stamp_cuebook_wordmark.mjs` once per final HTML. The stamp is idempotent and the launch linter rejects missing, displaced, or textual substitutes.
...
- Do not synthesize a structured evidence object inside this Skill. Its `EOBJ_*` ref, shape, source refs, method metadata, and reconciliation must already pass `$assemble-cuebook-viewpoint-data` and appear in `render_payload.evidence_object_refs`.
...
- `references/visual-direction-set-v1.schema.json`: direction-set contract.
- `references/cuebook-static-design-kernel-v1.md`: single execution order fused from Taste-Skill, Huashu-Design, Jakub Krehel's skills, and Emil Kowalski's skills.
- `references/logic-progression-v1.md`: source-linked argument graph and per-direction route contract.
- `references/creator-adaptive-palette-system-v1.md`: creator/content palette inference, three candidate strategies, anti-repetition, and accessibility rules.
- `references/creator-palette-presets-v1.json`: canonical palette registry for frontend and renderer use.
...
- `references/cuebook-visual-critique-v1.md`: finance-specific visual review rubric.
