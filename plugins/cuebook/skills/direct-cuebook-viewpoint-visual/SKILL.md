...
5. Unless the user already approved a direction, create **three complete HTML previews using the same meaning, bindings, and logic progression** through three independent design logics and three palette strategies:
   ...
   Before layout, write one `expression_recipe` per direction. Consume the three locked `candidate_jobs` from the expression plan: exactly one `fast_read`, one `proof`, and one `system`. For each recipe record the locked `market_relationship`, a compatible `argument_archetype`, a distinct `composition_archetype`, evidence shapes within the lock, primary/support grammars, proof bindings, and data requirements. Select the argument archetype that explains why the evidence matters to a trader, then choose compatible geometry and composition from the registry. Preserve all selected material bindings across the set. Do not turn three palettes into three supposed ideas, change the analytical relationship for visual variety, or swap jobs merely because another grammar is easier to render.
...
6. Make the three composition archetypes and spatial skeletons visibly different. At most one direction may be an editorial statement. A quantitative thesis needs at least one proof-led composition whose evidence field occupies roughly 55-72% of usable area. When comparison, structure, series, distribution, or payoff is the proof, a giant number is an annotation and cannot be the hero. Changing color, font, border radius, or label placement does not create another direction.
7. Stage original Noi files into the artifact-local `fonts/` directory with `scripts/stage_noi_font_assets.py`, then start from `assets/cuebook-viewpoint-canvas.html` and author freely. Evaluation may use a local Trial folder; production must use licensed non-Trial files and an opaque `license_ref`. Keep HTML and approved local assets network-free. Do not force content into starter slots.
...
python3 scripts/validate_visual_direction_set.py visual-direction-set-v1.json \
  --require-expression-recipes --require-finance-route
python3 scripts/stamp_cuebook_wordmark.py direction-a.html --background dark
python3 scripts/lint_launch_viewpoint_html.py direction-a.html
...
When the output feeds `$assemble-cuebook-publish-candidates`, all three directions are user-facing finished choices. Keep the set in `previewed` state until the frontend selects one.
...
- Run `scripts/stamp_cuebook_wordmark.py` once per final HTML. The stamp is idempotent and the launch linter rejects missing, displaced, or textual substitutes.
...
- Do not reuse the same DOM skeleton for all three directions.
...
- `scripts/validate_visual_direction_set.py`: structural and source-binding validator.
- `scripts/capture_html_viewpoint.cjs`: deterministic full and compact PNG capture.
