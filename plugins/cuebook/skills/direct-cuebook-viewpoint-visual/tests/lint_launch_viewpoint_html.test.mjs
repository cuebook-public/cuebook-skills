...
const mutations = [
  ...
  ["manual claim break", (source) => source.replace("earn an", "earn<br>an"), "CLAIM_MANUAL_BREAK"],
  ["unscoped disclosure", (source) => source.replace("</main>", "<small>Shareholder rights are limited</small></main>"), "UNSCOPED_TEXT"],
  ["generated copy", (source) => source.replace("</style>", '.x:after{content:"7/29 earnings"}</style>'), "GENERATED_COPY"],
  ["excess copy", (source) => source.replace("Watch usage at earnings", "word ".repeat(30)), "ROLE_BUDGET"],
  ...
  ["unlocked mutable price", (source) => source.replace("HOOD · 30D", "HOOD · current price 120 · 30D"), "MUTABLE_PRICE_UNLOCKED"],
];
...
test("compact verdict plus implication claim fits", () => {
  const source = validHtml().replace("I expect HOOD to earn an infrastructure premium", "Inventory beat: excess supply stays bearish");
...
test("a backend-bound price lock permits an explicitly labeled current price", () => {
  const source = validHtml()
    .replace("HOOD · 30D", "HOOD · current price 120 · 30D")
