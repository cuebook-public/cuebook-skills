...
- **settlement** (deferred until publication): an eligible single-asset `long` or `short` uses `single_asset_direction` automatically, with `threshold_bps: "0"`, `session_policy: "at_instant"`, and the exact creator-owned deadline. Record its provenance as `policy_default`. The creator's explicit publish confirmation confirms this standard direction rule together with the selected Frame; do not ask a second settlement question. Ask only when the creator requests a price-target or pair override.
...
Every creator-supplied field carries provenance: `stated` (present in the raw input), `elicited` (answered to a follow-up), or `inferred_confirmed` (inferred by the Skill and explicitly confirmed by the user). Server policy may deterministically supply the standard zero-threshold direction rule after publish intent; it must never manufacture an asset, direction, horizon, target, or pair.
...
7. **Confirm once.** Render one recap card — `asset · horizon · direction · optional target · one-line gist` — and get an explicit yes or an edit. For a subsequent “publish this” action, that explicit intent confirms both the frozen Frame and the standard direction settlement; do not display or confirm an internal session field. Edits reopen only the changed field.
...
- Never fill an asset, direction, horizon, target, pair, or creator rationale the user did not state or confirm. The only policy-derived launch values are the standard single-asset family and zero-bps threshold after explicit publish intent.
