...
Use `--ref v0.9.23` only when you intentionally want a tag-pinned install. The default `main` marketplace follows stable releases.
...
The four public fields stay stable while expression remains flexible. Generated visuals use Cuebook's recommended profile; a creator-supplied PNG, JPEG, or WebP keeps its native dimensions and aspect ratio. A provider-hosted interactive Artifact may sit behind the image, which remains the immutable poster and failure fallback. Cuebook does not mirror the live page, and a hosting domain is not treated as creator-model provenance.

Subject assets, an external Artifact, and Settlement are independent. A Frame can remain non-settling while still being discoverable by its exact subject assets, and all four Artifact × Settlement combinations are valid.
...
No Settlement, duration, or range band is the default. Explicit creator terms always win; if Settlement is chosen and a term is missing, Cuebook asks whether the creator wants to state it or receive one or two Cue-, catalyst-, history-, or volatility-informed proposals, and nothing proceeds until the creator accepts or edits a proposal. Creators never choose sessions, trading days, or next close. Cuebook then shows one complete Frame—the exact copy, actual image or Artifact poster, and any human Settlement rule—and asks whether to publish that expression or change it. “Publish this” both selects the displayed copy-to-image pair and authorizes its external write; there is no earlier copy-only confirmation.

## Creation Model

The Skill behaves like an attentive editor rather than exposing a sequence: it recognizes the creator's non-obvious kernel, adds one useful Cuebook connection when helpful, reveals one relationship visually, and asks once about the finished expression in ordinary conversation. The implementation keeps four layers separate:
...
4. **Publication** — when the complete Frame is shown, the client silently reserves one upload, PUTs the frozen generated or creator-supplied image, and completes media processing in quarantine while the creator decides. Explicit publication intent then triggers one atomic publish call. Its successful typed result ends the network flow; there is no second confirmation, receipt parsing, reconciliation, generated web page, canonical link, or readback. The creator receives a specific acknowledgment of the idea that was preserved and returns to Cuebook App. Cuebook chooses at most one natural continuation: a qualified memory candidate, sharing from the App with another AI, another signal, or explicit opt-in to a later simulated Paper Trade.
...
Each generated preview renders one recommended **1866 × 1200** publication PNG from the internally frozen draft. A creator-provided PNG, JPEG, or WebP is uploaded at its native dimensions without crop, stretch, padding, or restamping. Cuebook binds one publication image in either route; the Skill does not create separate compact, web, thumbnail, or OG authoring assets.

The generated master is authored against its 622 × 400 phone display box and rasterized at 3x: one dominant geometry, at most three reader-essential groups, a 20 px primary and 16 px secondary essential type floor, minimal provenance, one material dated value when price matters, and one visible future check. The server derives display renditions without changing creator media's source aspect ratio.
...
Layering, extension-pattern criteria, routing truth hierarchy, the dormant
capability registry, and budget/drift governance live in
[ARCHITECTURE.md](ARCHITECTURE.md). Structural changes update that contract in
the same commit.
