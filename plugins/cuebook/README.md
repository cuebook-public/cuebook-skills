...
Use `--ref v0.9.8` only when you intentionally want a tag-pinned install. The default `main` marketplace follows stable releases.
...
1. **Creator meaning** — the claim, mechanism, time horizon, and next observable remain creator-owned. Cuebook never fills a missing horizon from a preset. When the creator asks for help choosing, relevant Cues and catalysts may inform at most two proposals, but the creator must accept or edit one. After asset resolution, at most one aligned and one contrasting or adjacent Cue may be offered as optional thought anchors; only adopted additions enter the viewpoint.
