#!/usr/bin/env node
// Validate and run the Cuebook fast-expression V2 path. V2 keeps the first
// preview lightweight while adding typed argument, time, curve, layout, and
// per-element source-binding semantics.
...
import {
  ...
  renderExpressionSvg,
} from "./render_frame_expression_v2.mjs";
...
const JOB_SCHEMA = JSON.parse(readFileSync(new URL("../references/frame-preview-fast-job-v2.schema.json", import.meta.url), "utf8"));
...
export function validateV2Job(job) {
...
async function renderCandidate(expression, candidate, outputRoot, dependencies) {
  ...
  const expressionPath = path.join(candidateDir, "visual-argument-fast-v2.json");
  ...
  if (!existsSync(pngPath)) throw new Error("V2 rasterizer did not produce the publication PNG.");
  return {
...
export async function runFastPreviewV2Job(job, outputDir, dependencies = {}) {
  const startedAt = Date.now();
  const validation = validateV2Job(job);
  if (!validation.valid) failValidation("Fast preview V2 job", validation.errors);
  ...
  const report = {
    schema_version: "frame-preview-fast-run-report-v2",
...
async function main() {
  ...
  if (!input || !outputDir || process.argv.length !== 4) {
    process.stderr.write("usage: run_fast_preview_v2.mjs frame-preview-fast-job-v2.json output-dir\n");
    process.exitCode = 2;
    return;
  }
  const job = JSON.parse(readFileSync(input, "utf8"));
  const result = await runFastPreviewV2Job(job, outputDir);
