...
class DirectionSetTests(unittest.TestCase):
    ...
    def test_ordered_axis_grammar_requires_axis_integrity(self) -> None:
        payload = base_payload()
        recipe = payload["directions"][1]["expression_recipe"]
        recipe.update({"candidate_job": "evidence_proof", "primary_grammar": "maturity_profile", "evidence_shapes": ["term_structure"]})
        codes = {item["code"] for item in VALIDATOR.validate(payload)}
        self.assertIn("AXIS_INTEGRITY", codes)

        recipe["axis_integrity"] = "ordinal_gap_marked"
        codes = {item["code"] for item in VALIDATOR.validate(payload)}
        self.assertNotIn("AXIS_INTEGRITY", codes)
