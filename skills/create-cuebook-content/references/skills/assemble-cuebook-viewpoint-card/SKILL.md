...
2. Compile `MarketViewSemanticsV1`, then build one `../plan-cuebook-creator-expression/SKILL.md`. Compose a trading thesis only when the semantic layer contains real trade intent. Use `../build-market-research-pack/SKILL.md` for selected or recommended news and context.
3. Resolve enrichment blocks:
   - text through `../render-cuebook-market-post/SKILL.md` or preserved creator text;
   - news through sourced ResearchPack facts;
   - the primary compact argument through `../assemble-cuebook-viewpoint-data/SKILL.md` and `../render-cuebook-viewpoint-visual/SKILL.md`;
   - oversized branching logic through the legacy `../compile-cuebook-visual-argument/SKILL.md` and `../render-cuebook-logic-card/SKILL.md` only when it cannot fit the compact grammar;
   - legacy data-led figure through `../render-cuebook-market-figure/SKILL.md` only for compatibility;
   - chart through `../render-cuebook-thesis-chart/SKILL.md`;
   - indicator through `../compute-cuebook-market-indicators/SKILL.md`;
   - settlement through `../compile-cuebook-settlement-claim/SKILL.md`.
