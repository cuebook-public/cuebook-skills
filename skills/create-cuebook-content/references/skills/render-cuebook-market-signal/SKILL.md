...
- Use `../render-cuebook-market-figure/SKILL.md` when a sourced curve, relationship, or capital-flow sequence is available.
- Use `../render-cuebook-logic-card/SKILL.md` when the argument needs two or more causal, comparative, or scenario nodes and no quantitative curve is necessary.

## Workflow

1. Freeze one source-bounded signal and a decision cutoff.
2. Run `../classify-cuebook-trading-logic/SKILL.md`. Use its action line, reason line, horizon, and two to four public tags. Keep `evidence_boundary` private.
