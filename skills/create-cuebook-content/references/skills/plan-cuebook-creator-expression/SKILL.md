...
description: Plan a required MarketViewSemanticsV1 plus optional ResearchPackV1, TradingThesisV1, TradeLogicProfileV1, and ProfileV1 into CreatorExpressionPlanV1, the semantically locked shared bridge for one Frame title, one body, and one paired image. Use after market-view meaning and ownership are known and before Frame copy or visual rendering; use especially to preserve a creator's viewpoint, reconstruct its supporting logic, identify missing support, select a narrative engine, preserve internal authorship provenance, set VoiceSpec and anti-AI-language controls, divide work between text and image, or assess settlement eligibility. Do not use to research missing facts, create or adopt a trade, compile or settle a claim, render final copy or art, create social-platform variants, imitate a living commentator, or relabel an external source view as the current creator's experience.
...
- Pass the same plan and fingerprint hash to `../render-cuebook-market-post/SKILL.md` for Frame-sized body text. The candidate assembler derives the title and binds the selected image; no social or selector-only public derivative is created.
