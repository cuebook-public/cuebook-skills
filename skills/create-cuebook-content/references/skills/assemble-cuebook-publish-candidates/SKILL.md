...
3. Invoke `../render-cuebook-market-post/SKILL.md` once in batch candidate mode. Return three reasoning-complete `PostV1` siblings from the same plan with distinct expression briefs:
...
10. Validate with `scripts/validate_publish_candidate_set.py`, including `--asset-root` when local previews are present.

```bash
python3 scripts/validate_publish_candidate_set.py publish-candidate-set-v1.json --asset-root ./output
```
...
- `scripts/validate_publish_candidate_set.py`: copy-budget, fingerprint, visual, settlement, and state validator.
- `tests/test_validate_publish_candidate_set.py`: regression tests.
