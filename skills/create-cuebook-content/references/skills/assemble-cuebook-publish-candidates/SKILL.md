...
4. For an explicit three-alternative request, invoke `../render-cuebook-market-post/SKILL.md` once in batch mode. Generate `conviction`, `evidence`, and `catalyst` or `mechanism` siblings from one meaning lock and one fact ledger. Retry only a failed sibling.
