...
1. Resolve the assigned structured-media output from ContentRecipeV1. Apply channel, format, count, target context, language, flavor, and selected media/profile refs. For coordinated outputs, call `../plan-market-content-program/SKILL.md`, render only the assigned item, and preserve recipe/program/content-item/opportunity/input-artifact refs in `lineage`.
...
9. Create an asset plan with explicit rights status; do not reuse source media merely because it is public. Resolve data through `../assemble-cuebook-viewpoint-data/SKILL.md`, then call `../render-cuebook-viewpoint-visual/SKILL.md` for the primary compact argument and preserve its ref in both lineage and `asset_plan.artifact_ref`. Route only a long price history to `../render-cuebook-thesis-chart/SKILL.md`, and only an oversized branching argument to the legacy visual-argument and logic-card pair.
...
```
13. For owned web, rerun selected `../optimize-cuebook-market-seo/SKILL.md` and `../optimize-cuebook-market-geo/SKILL.md` modules in `preflight` against the final artifact. Then route the final `MediaPackageV1` and sidecar references to `../prepare-market-content-release/SKILL.md`; this skill performs no account or platform operation.
