...
> Module resources are rooted at `references/modules/compose-cuebook-trading-thesis/` from the public Skill directory.
...
10. To freeze, set `lifecycle_state` to `frozen`, compute the canonical hash with `references/modules/compose-cuebook-trading-thesis/scripts/validate_trading_thesis.mjs --print-canonical-hash`, store it in `lineage.canonical_hash`, and validate again. Any substantive change after freeze becomes a new revision or linked thesis.
11. Route the frozen thesis downstream. Use `references/modules/plan-market-content-program.md` for a release family, `references/modules/render-cuebook-market-post.md` for compact text, `references/modules/render-cuebook-market-media.md` for structured media, `references/modules/render-cuebook-thesis-chart.md` for a claim-bound market chart, `references/modules/compute-cuebook-market-indicators.md` for deterministic OHLCV evidence, `references/modules/assemble-cuebook-viewpoint-card.md` for the product preview, and `references/modules/prepare-market-content-release.md` for final handoff.

Read `references/modules/compose-cuebook-trading-thesis/references/thesis-protocol.md` before composing or revising a thesis.
...
Return the exact shape in `references/modules/compose-cuebook-trading-thesis/references/trading-thesis-v1.schema.json`. Validate it with:

```bash
node references/modules/compose-cuebook-trading-thesis/scripts/validate_trading_thesis.mjs trading-thesis-v1.json
```
...
```bash
node references/modules/compose-cuebook-trading-thesis/scripts/validate_trading_thesis.mjs trading-thesis-v1.json --print-canonical-hash
```

## Resources

- `references/modules/compose-cuebook-trading-thesis/references/thesis-protocol.md`: product semantics, lifecycle, settlement, and rendering boundary.
- `references/modules/compose-cuebook-trading-thesis/references/trading-thesis-v1.schema.json`: authoritative declaration contract.
- `references/modules/compose-cuebook-trading-thesis/scripts/validate_trading_thesis.mjs`: deterministic cross-reference, cutoff, resolution, disclosure, and hash checks.
- `references/modules/compose-cuebook-trading-thesis/evals/trigger_cases.json`: routing examples.
- `references/modules/compose-cuebook-trading-thesis/evals/rubric.md`: quality rubric for forward evaluation.
