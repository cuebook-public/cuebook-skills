...
> Module resources are rooted at `references/modules/render-cuebook-viewpoint-visual/` from the public Skill directory.
# Render Cuebook Viewpoint Visual

Turn an approved visual direction into one exact 2488 x 1056 publication master. Author the HTML at 1244 x 528, use a 2x capture, and inspect that same composition at a 622 x 264 display size. Visual form is flexible; market meaning and source fidelity are not.
...
1. Require a selected `VisualDirectionSetV1`. If it does not exist, run `references/modules/direct-cuebook-viewpoint-visual.md` first and inspect its three real previews.
...
4. Require production `data-font-license-mode`, an artifact-local `fonts/font-assets-v1.json` from the director Skill's staging script, and the canonical `data-cuebook-wordmark="v1"` SVG in the bottom-right safe zone. Trial files may be used for local evaluation but are never release-eligible. If polish changed the HTML, rerun `references/modules/direct-cuebook-viewpoint-visual/scripts/stamp_cuebook_wordmark.mjs` as the last mutation, followed by its launch linter.
5. Capture one output with `references/modules/direct-cuebook-viewpoint-visual/scripts/capture_html_viewpoint.cjs`; `viewpoint-2488.png` must be exactly 2488 x 1056 and match the direction's `preview_ref`. Do not create compact, web, thumbnail, or OG files.
6. Inspect the publication PNG at full and 622 x 264 display sizes and run the director Skill's rendered audit. Confirm at least one `Cuebook Noi` face loaded, all visible non-brand copy leads with the approved stack, and CJK fallback did not change hierarchy or wrapping. Re-run the Cuebook visual critique after any typography, spacing, or composition change. Return to the layout Skill when the approved structure itself needs revision.
7. Run `references/modules/render-cuebook-viewpoint-visual/scripts/finalize_wide_viewpoint.mjs` to emit `ViewpointVisualV1` with `render_profile: wide_2488`, the selected `VDIR_` ref, HTML hash, production font-manifest hash, publication PNG hash, palette preset, argument pattern, and source bindings. Freeze that manifest in the release artifact.

```bash
node references/modules/render-cuebook-viewpoint-visual/scripts/finalize_wide_viewpoint.mjs visual-direction-set-v1.json \
  --asset-root . \
...
1. Read `references/modules/render-cuebook-viewpoint-visual/references/visual-grammar-reference.md` and `references/modules/render-cuebook-viewpoint-visual/references/cuebook-editorial-signal-v2.md`.
...
```bash
node references/modules/render-cuebook-viewpoint-visual/scripts/validate_viewpoint_visual.mjs viewpoint-visual-spec-v1.json
node references/modules/render-cuebook-viewpoint-visual/scripts/render_viewpoint_visual.mjs viewpoint-visual-spec-v1.json --output-dir ./viewpoint-visual
node references/modules/render-cuebook-viewpoint-visual/scripts/validate_viewpoint_visual.mjs ./viewpoint-visual/viewpoint-visual-v1.json --asset-root ./viewpoint-visual
```

## Public Image Contract

- A reader at 622px must identify the claim before the taxonomy, source, or brand.
- Keep 2–3 essential prose groups, mark each with `data-essential-copy-group`, and keep their effective type at 18px or larger in the 622 x 264 audit.
...
Route to `references/modules/render-cuebook-thesis-chart.md` when OHLC, volume, multiple indicators, entry/invalidation/target geometry, or settlement tracking is the main evidence. A full chart can then become the hero primitive inside a directed composition; do not redraw it as a decorative line.
...
- Reject clipped text, unresolved placeholders, external network assets, and masters that fail either full-size or 622 x 264 display-scale inspection.
...
- `references/modules/direct-cuebook-viewpoint-visual.md`: default art-direction and three-preview workflow.
- `references/modules/direct-cuebook-viewpoint-visual/assets/cuebook-wordmark.svg`: single canonical brand source used by both HTML and compatibility SVG routes.
- `references/modules/direct-cuebook-viewpoint-visual/references/typography-benchmark-profiles-v1.json`: launch font profile and benchmark brand-distance contract.
- `references/modules/direct-cuebook-viewpoint-visual/scripts/stage_noi_font_assets.mjs`: artifact-local font staging and hashed license-mode manifest.
- `references/modules/render-cuebook-viewpoint-visual/references/viewpoint-visual-spec-v1.schema.json`: deterministic compatibility input.
- `references/modules/render-cuebook-viewpoint-visual/references/viewpoint-visual-v1.schema.json`: unified launch and compatibility output manifest; `wide_2488` is the publication profile.
- `references/modules/render-cuebook-viewpoint-visual/references/cuebook-visual-tokens-v1.json`: shared Cuebook tokens.
- `references/modules/render-cuebook-viewpoint-visual/references/cuebook-editorial-signal-v2.md`: legacy 11-composition language.
- `references/modules/render-cuebook-viewpoint-visual/scripts/render_viewpoint_visual.mjs`: deterministic SVG compatibility renderer.
- `references/modules/render-cuebook-viewpoint-visual/scripts/finalize_wide_viewpoint.mjs`: deterministic selected-direction finalizer for the launch profile.
- `references/modules/render-cuebook-viewpoint-visual/scripts/rasterize_viewpoint_visual.cjs`: deterministic SVG derivative publisher.
