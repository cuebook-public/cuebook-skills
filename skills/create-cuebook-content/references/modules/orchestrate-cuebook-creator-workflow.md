...
17. **Receipt terminal**: require and validate the publish receipt's versioned Frame ref, then stop the network flow. Do not call `get_frame`, browse or curl the canonical page, inspect web HTML or metadata, or expose a URL. Return only “已发布，去 Cuebook App 看。” to the creator. `get_frame` remains a Query capability for a later explicit read, never an automatic publication check.
