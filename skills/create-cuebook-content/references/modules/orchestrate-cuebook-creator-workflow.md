...
12. **Selected viewpoint render**: execute the locked visual path only for retained directions. Recompose the selected fast-preview template with production fonts, full and compact derivatives, capture, audit, bindings, and manifest lineage. Add OG only for public/unlisted visibility. Three structurally different release directions are required only for an explicit alternative request. Use `references/modules/render-cuebook-thesis-chart.md` only when the selected route genuinely needs OHLC/K-line, long history, forming bars, explicit levels, or a settlement clock.
