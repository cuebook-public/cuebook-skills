...
> Module resources are rooted at `references/modules/compose-cuebook-content-recipe/` from the public Skill directory.
...
2. Load `references/modules/compose-cuebook-content-recipe/references/skill-catalog-v1.json`; pin its `catalog_version` in the recipe.
3. When running inside the Cuebook plugin, resolve frontend choices from `assets/plugin/creation-menu-v1.json`. The menu is presentation metadata; SkillCatalogV1 remains authoritative for dependencies, versions, visibility, and execution.
...
8. Validate the recipe with `node references/modules/compose-cuebook-content-recipe/scripts/validate_content_recipe.mjs <recipe.json>` before sending it to `references/modules/orchestrate-cuebook-creator-workflow.md`.

Read `references/modules/compose-cuebook-content-recipe/references/recipe-method.md` when resolving modes, output channels, presets, or skill dependencies.
...
Return `ContentRecipeV1` from `references/modules/compose-cuebook-content-recipe/references/content-recipe-v1.schema.json`. A valid recipe contains:
...
- `references/modules/compose-cuebook-content-recipe/references/content-recipe-v1.schema.json`: authoritative recipe contract.
- `references/modules/compose-cuebook-content-recipe/references/skill-catalog-v1.json`: canonical frontend skill and preset registry.
- `references/modules/compose-cuebook-content-recipe/references/skill-catalog-v1.schema.json`: registry contract.
- `references/modules/compose-cuebook-content-recipe/references/recipe-method.md`: selection modes, ingredient roles, and resolution rules.
- `assets/plugin/creation-menu-v1.json`: product-facing ingredient, reasoning, voice, visual, settlement, and output choices.
- `references/modules/compose-cuebook-content-recipe/scripts/validate_content_recipe.mjs`: recipe, feed, opportunity, and catalog checks.
- `references/modules/compose-cuebook-content-recipe/scripts/validate_skill_catalog.mjs`: catalog dependency, visibility, version, and path checks.
- `references/modules/compose-cuebook-content-recipe/evals/trigger_cases.json`, `references/modules/compose-cuebook-content-recipe/evals/rubric.md`, and `references/modules/compose-cuebook-content-recipe/evals/failure_cases.md`: routing and quality evaluation.
