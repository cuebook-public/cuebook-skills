...
- Render exactly one 2488 × 1056 publication PNG. It is the same image used in Feed and detail views; design it to remain legible when the host scales it to 622 × 264. HTML templates mark 2–3 reader-essential prose groups with stable `data-essential-copy-group` names and keep their effective phone-display type at 18 px or larger; a finished bitmap's image reviewer verifies the same attention budget directly from pixels. Do not render, upload, bind, or present separate compact, web, thumbnail, or OG files.
- Use one dominant geometry, at most two essential copy groups, a mobile-effective 11 px type floor at quarter scale, minimal provenance, and one future check. Keep source, as-of time, and transform honest and legible. Synthetic fixtures are visibly non-publishable and can never masquerade as market observations.
...
After selection, freeze the exact title, body, image, creator meaning, evidence refs, and image bytes. Never silently rewrite them. Run `references/modules/orchestrate-cuebook-creator-workflow.md` only now and materialize only the contracts needed for the chosen Frame. Reuse the frozen 2488 × 1056 PNG as the sole publication master; do not produce release derivatives. `finished_bitmap` is the default for a selected Fast Preview; HTML and font files are optional provenance and never a publication gate. Use `cuebook_template` only when the selected design is intentionally re-rendered from licensed template assets.
