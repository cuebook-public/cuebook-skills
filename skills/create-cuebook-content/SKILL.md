...
2. Run the **One-Round Creator Interview** below before querying, generating, or asking anything about price or settlement. Preserve the exact answer in working context and distill useful parts into the claim, mechanism, evidence search, next watch, and visual argument.
...
## One-Round Creator Interview

- Extract context the user already supplied; never ask them to repeat it. Once subject, direction, and horizon are known, offer **one** compact follow-up that invites any triggering news or event, market clue or signal, and the intuition or inspiration they most want preserved. Ask naturally, not as a checklist, and use no more than two short questions in the same turn.
- A good prompt is: 「为了把这更像你的判断：是什么新闻、盘面线索或 signal 让你注意到它？你最想保留的直觉是什么？没有更多也没关系，回‘就按这个做’我就马上生成。」 Adapt the wording to the creator's language and what they already shared.
...
- Never ask the creator to prove or defend the view. Use creator-supplied news as a search lead until Cuebook verifies it; use clues and signals as candidate evidence beats; use intuition as the creator's hypothesis, angle, or visual mechanism. Preserve their authorship and do not silently convert inference into observed fact.
