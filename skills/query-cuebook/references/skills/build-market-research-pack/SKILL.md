...
2. If the input is a Cuebook cue, run `../validate-cuebook-projection/SKILL.md` first. Stop on `reject`; preserve every caution and repair in the research gaps.
3. For a Cuebook cue that is not rejected, run `../route-cuebook-narrative/SKILL.md`. Use `event_type`, `reasoning_lenses`, and `required_context` to select research modules; the route does not approve evidence or publication.
